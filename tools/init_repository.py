...
def main() -> int:
    parser = argparse.ArgumentParser(description="MDE 저장소의 기본 폴더와 문서 체계를 생성합니다.")
    parser.add_argument("--root", type=Path, default=default_root(), help="MDE 저장소 루트 경로")
    parser.add_argument("--force", action="store_true", help="생성기가 관리하는 템플릿 파일을 덮어씁니다.")
    ...
    for item in changed:
        print(f"  [{item.action}] {item.path.relative_to(args.root.expanduser().resolve())}")
    return 0
